...
The  \iket{1}~\ilra~\iket{2}  transition,  $\omega_{21}$,  is  taken  with  a  network
...
higher $\omega_{21}$ at higher magnetic flux numbers.  \red{Explain resonant emission,
  destructive interference and show graph the transmission dip}

\begin{figure}[h]
  \centering \includegraphics[height = 7cm]{transmission_combined.png}
  \caption{To map  out the twin qubit  spectrum, a network analyser  measures the
    power transmission coefficient, \iabsSquared{t} }
...
state \iket{1}.  This will  show up in an improved transmission  of the signal at
...
Fig.~\ref{fig:experiment}. \red{parameters?}
...
\begin{figure}[h]
  ...
  \caption{\small Transition  frequencies between levels  \iket{1}\ilra \iket{2},
    $ \omega_{21}  $ (blue), and \iket{2}  \ilra \iket{3}, $ \omega_{32}$  (red).  Readings
...
\noindent where \iabs{C} is the capacitance of the outer JJs.  The total charging
...
Each of the  5 JJs each contribute $ E_{Ji}\left(1  - \cos(\varphi_i)\right) $ to
the potential term.  The two junction phases unspecified by $ \vec{\varphi} $ are
pinned  by  the  flux  quantization  condition for  the  left  and  right  loops,
$  \sum_{i}^{\text{loop}} \varphi_i  = 2\pi  n,  n \in  \mathbb{Z}}$, which  have
...
which  the  kinetic  terms are  on  the  diagonal  and  the potential  terms  are
...
SEM image in Fig.~\ref{fig:setup}.
