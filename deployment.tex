...
from the SEM image in Fig.~\ref{fig:setup}.
 ...
 \begin{figure}[h!]
   ...
   \caption{Rabi oscillation measurements are taken at the degeneracy flux bias, $ \Phi_0/2 $, by
     driving the qubit with resonant microwaves,  $\omega_{\text{VNA}} = \omega_{21}$ for different time
     periods,  $ dt  $,  and monitoring  the  signal in  the output  line  \cite{rabi}. The  a
     decoherence  time of  $ T_\varphi  = \iunit{42}{ns}  $ is  extracted from  the decay  envelope,
     $ e^{-dt/\tau_\varphi} $, of the the oscillations. \label{fig:rabi}}
 \end{figure}

 At the  degeneracy flux bias $  \Phi = (n +  \frac{1}{2})\Phi_0, n\in\mathbb{Z} $, the  energy levels
